...
# use a relative import so that the package can work as a namespace
# without requiring `codebase_rag` to be installed as a top-level module.
...
class GraphUpdater:
    ...
    def _ingest_top_level_functions(
        ...
        # Use a QueryCursor because Query objects no longer have a captures()
        # method in newer tree‑sitter releases.
        cursor = QueryCursor(lang_queries["functions"])
    ...
    def _ingest_classes_and_methods(
        ...
        for class_node in class_nodes:
            ...
            cursor = QueryCursor(lang_queries["functions"])
            method_captures = cursor.captures(body_node)
    ...
    def _process_calls_in_classes(
        ...
        cursor = QueryCursor(lang_queries["classes"])
    ...
    def _ingest_function_calls(
        ...
        # ``calls_query`` might be None if the language doesn't specify calls.
        call_captures = {}
        if calls_query:
            cursor = QueryCursor(calls_query)
            call_captures = cursor.captures(caller_node)
