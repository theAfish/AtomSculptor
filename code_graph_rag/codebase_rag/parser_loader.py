    ...
    for lang_name, lang_config in LANGUAGE_CONFIGS.items():
        ...
        if lang_lib:
            try:
                ...
                # Compile queries using the new Query constructor (language.query is deprecated)
                ...
                # Tree‑sitter 0.21+ no longer exposes captures() on Query itself; we
                # will use QueryCursor later when executing queries.  But we still
                # store a Query object here for reference and to keep the existing
                # data structure.
